...
public class TileEntityBChest extends TileEntityLockableLoot implements ITickable 
{
	...
	{
		return this.hasCustomName() ? this.customName : "Birch Chest";
	}
	...
	{
		return new ContainerBChest(playerInventory, this, playerIn);
	}
	...
	public void update() 
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerBChest)
                {
                    if (((ContainerBChest)entityplayer.openContainer).getChestInventory() == this)
                    {
